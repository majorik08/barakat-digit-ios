...
class HistoryInfoItemView: UIView {
    ...
    init(withNoLine: CGRect) {
        super.init(frame: withNoLine)
        self.translatesAutoresizingMaskIntoConstraints = false
        self.backgroundColor = .clear
        self.addSubview(self.titleLabel)
        self.addSubview(self.infoLabel)
        NSLayoutConstraint.activate([
            ...
            self.titleLabel.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -4),
            self.infoLabel.leftAnchor.constraint(equalTo: self.titleLabel.rightAnchor, constant: 10),
            self.infoLabel.topAnchor.constraint(equalTo: self.topAnchor, constant: 4),
            self.infoLabel.rightAnchor.constraint(equalTo: self.rightAnchor, constant: 0),
            self.infoLabel.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -4),
        ])
    }
